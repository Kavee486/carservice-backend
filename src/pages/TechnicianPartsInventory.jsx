...
import { Package, Search, Filter, Box, AlertTriangle, RefreshCw, X } from 'lucide-react';
...
const TechnicianPartsInventory = () => {
  ...
  useEffect(() => {
    if (parts && Array.isArray(parts)) {
      const outOfStockItems = parts.filter(part => parseInt(part?.P_StockQty || 0) === 0);
      const newOutOfStockCount = outOfStockItems.length;
      
      setOutOfStockCount(newOutOfStockCount);
      
      // Show alert if there are out-of-stock items and we haven't shown the initial alert yet
      if (newOutOfStockCount > 0 && !hasShownInitialAlert) {
  ...
  const handleRefresh = () => {
    dispatch(GetAllPartsInventory());
    dispatch(GetAllCategories());
    // Reset the initial alert flag so it shows again if there are still out-of-stock items
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 px-4 md:px-6 py-4 md:py-6">
      ...
      {showOutOfStockAlert && outOfStockCount > 0 && (
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <div className="mt-1 text-sm text-red-700">
                  <p>
                    You have {outOfStockCount} part{outOfStockCount !== 1 ? 's' : ''} that are out of stock.
                  </p>
                </div>
                <div className="mt-2">
                  <button
                    onClick={handleCloseAlert}
                    className="text-sm bg-red-100 hover:bg-red-200 text-red-800 font-medium py-1 px-3 rounded-lg transition-colors duration-200"
                  >
                    View Details
                  </button>
                </div>
            ...
            <div className="flex items-center space-x-2">
              {outOfStockCount > 0 && (
                <button
                  ...
                  className="bg-red-500 hover:bg-red-600 text-white border-0 rounded-lg h-10 px-4 text-sm font-medium flex items-center transition-colors duration-200"
                >
              ...
              <button
                onClick={handleRefresh}
                className="bg-white bg-opacity-20 hover:bg-opacity-30 text-white border-0 rounded-lg h-10 px-4 text-sm font-medium backdrop-blur-sm flex items-center transition-colors duration-200"
              >
                <RefreshCw className="h-4 w-4 mr-2" />
                Refresh
              </button>
